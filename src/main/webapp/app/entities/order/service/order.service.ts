...
type RestOf<T extends IOrder | NewOrder> = Omit<T, 'fechaOperacion'> & {
...
export class OrderService {
  ...
  protected convertDateFromClient<T extends IOrder | NewOrder | PartialUpdateOrder>(order: T): RestOf<T> {
    return {
      ...order,
      fechaOperacion: order.fechaOperacion?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restOrder: RestOrder): IOrder {
    return {
      ...restOrder,
      fechaOperacion: restOrder.fechaOperacion ? dayjs(restOrder.fechaOperacion) : undefined,
    };
  }
